...
from models import QueryLog, Feedback, User
...
class EvaluationService:
    ...
    async def evaluate_response(self, request: EvaluationRequest) -> EvaluationResult:
        ...
        try:
            ...
            context_str = "\n".join(request.context_sources) if request.context_sources else "No context provided"
            ...
            confidence_interval = self._calculate_confidence_interval(scores, evaluation_data["confidence"])
    ...
    async def _get_llm_evaluation(self, prompt: str, max_retries: int = 3) -> Dict:
        ...
        for attempt in range(max_retries):
            try:
                ...
                required_fields = ["relevance_score", "accuracy_score", "clarity_score", "completeness_score", "reasoning", "confidence"]
                if not all(field in evaluation_data for field in required_fields):
                    raise ValueError(f"Missing required fields in evaluation response")
                
                # Validate score ranges
                for score_field in ["relevance_score", "accuracy_score", "clarity_score", "completeness_score"]:
                ...
                if attempt == max_retries - 1:
                    raise Exception(f"Failed to get valid evaluation after {max_retries} attempts")
                await asyncio.sleep(1)  # Brief delay before retry
    
    def _calculate_confidence_interval(self, scores: List[float], evaluator_confidence: float) -> Tuple[float, float]:
...
class ABTestingService:
    ...
    def create_ab_test(self, config: ABTestConfig) -> bool:
        ...
        logger.info(f"Created A/B test '{config.test_name}' requiring {required_sample} samples per variant")
        return True
    ...
    def record_result(self, test_name: str, variant: str, user_id: int, outcome_metric: float):
    ...
    def analyze_test_results(self, test_name: str) -> Dict:
        ...
        control_results = [r["outcome"] for r in results if r["variant"] == config.control_version]
        treatment_results = [r["outcome"] for r in results if r["variant"] == config.treatment_version]
        
        if len(control_results) < 30 or len(treatment_results) < 30:
            return {"status": "insufficient_data", "message": "Need at least 30 samples per variant"}
        
        # Perform t-test
        t_stat, p_value = stats.ttest_ind(treatment_results, control_results)
        
        # Calculate effect size (Cohen's d)
        pooled_std = np.sqrt(((len(control_results) - 1) * np.var(control_results, ddof=1) + 
        ...
        return {
            ...
            "lift": ((np.mean(treatment_results) - np.mean(control_results)) / np.mean(control_results)) * 100,
            "p_value": p_value,
            ...
            "recommendation": "deploy" if (is_significant and np.mean(treatment_results) > np.mean(control_results)) else "no_change"
        }
...
class MonitoringService:
    ...
    def get_performance_metrics(self, db: Session, days: int = 7) -> PerformanceMetrics:
        ...
        try:
            ...
            # Calculate metrics
            response_times = [q.processing_time for q in recent_queries if q.processing_time]
            ...
            return PerformanceMetrics(
                ...
                avg_quality_score=np.mean(confidence_scores) if confidence_scores else 0.0,
                total_queries=len(recent_queries),
                success_rate=len(successful_queries) / len(recent_queries) if recent_queries else 0.0,
    ...
    def _calculate_retrieval_accuracy(self, queries: List[QueryLog]) -> float:
        ...
        high_quality_queries = [
            q for q in queries 
            if q.confidence_score and q.confidence_score > 0.7
        ]
