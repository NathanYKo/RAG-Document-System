...
from utils import (
    validate_email, validate_password, format_file_size, format_timestamp,
    get_status_color, get_confidence_color, highlight_text, truncate_text,
    SessionManager, apply_custom_css
)
...
class APIClient:
        ...
        if response.status_code == 200 or response.status_code == 201:
            ...
            if 'detail' in error_data:
                if isinstance(error_data['detail'], list):
                    errors = [f"• {err.get('msg', str(err))}" for err in error_data['detail']]
    ...
    def submit_feedback(self, query_log_id: int, rating: int, 
                       comment: str = "", feedback_type: str = "general") -> Optional[Dict]:
        ...
        if SessionManager.is_authenticated():
            ...
            with st.expander("👤 Profile Stats", expanded=False):
                ...
                if user_data.get('avg_confidence_score'):
                    st.metric("Avg. Confidence", f"{user_data['avg_confidence_score']:.2f}")
...
def render_login_page(api_client: APIClient):
    ...
    try:
        ...
        st.error("❌ Cannot connect to RAG System. Please check if the backend is running.")
        return
                    ...
                    with st.spinner("🔐 Authenticating..."):
                        ...
                        if auth_data:
                            ...
                            if profile_data:
                                ...
                                st.session_state.username = profile_data.get('username', username)  # FIXED
    ...
    with tab2:
        ...
        with st.form("register_form"):
            new_username = st.text_input("Username*", placeholder="Choose a username (3-50 chars)")
                ...
                if not all([new_username, new_password, confirm_password, agree_terms]):
                    ...
                    if not is_valid:
                        ...
                        with st.spinner("📝 Creating account..."):
                            result = api_client.register(new_username, new_email or None, new_password)
                            if result:
...
def render_document_sources(sources: List[Dict], query: str):
    ...
    for i, source in enumerate(sources, 1):
        ...
        content = source.get('content', source.get('text', source.get('chunk_text', '')))
        ...
        with progress_container:
            ...
            try:
                ...
                if result:
                    ...
                    if result.get('id'):
                        render_feedback_section(api_client, result.get('id'), answer_text)
    ...
    if st.session_state.query_history:
        ...
        for i, item in enumerate(reversed(st.session_state.query_history[-5:]), 1):
            with st.expander(f"Query {i}: {item['query'][:50]}...", expanded=False):
                ...
                answer = item['result'].get('answer', item['result'].get('result', 'N/A'))
                st.markdown(f"**Answer:** {answer}")
                st.caption(f"Asked at {item['timestamp'].strftime('%Y-%m-%d %H:%M:%S')}")
...
def render_feedback_section(api_client: APIClient, query_log_id: int, answer_text: str):
    ...
    with col1:
        rating = st.slider("Overall Rating", 1, 5, 3, help="Rate the quality of this response")
    ...
    comment = st.text_area("Additional Comments (Optional)", placeholder="Tell us how we can improve...")
...
def render_documents_page(api_client: APIClient):
    ...
    with col2:
        ...
        chunk_size = st.slider("Chunk Size", 500, 2000, Config.DEFAULT_CHUNK_SIZE, step=100)
        chunk_overlap = st.slider("Overlap", 50, 500, Config.DEFAULT_CHUNK_OVERLAP, step=50)
            ...
            with upload_container:
                ...
                try:
                    ...
                    if file.size > Config.MAX_FILE_SIZE_MB * 1024 * 1024:
                        st.error(f"❌ File {file.name} is too large (max {Config.MAX_FILE_SIZE_MB}MB)")
                        continue
    ...
    if documents:
        ...
        if status_filter != "All":
            filtered_docs = [doc for doc in filtered_docs if doc['processing_status'] == status_filter]
...
def render_analytics_page(api_client: APIClient):
    ...
    if st.session_state.query_history:
        ...
        with col1:
            ...
            chart = alt.Chart(query_df).mark_line(point=True).encode(
        ...
        with col2:
            ...
            chart = alt.Chart(query_df).mark_bar().encode(
                ...
                title="Query Processing Time Distribution",
                width=400,
                height=300
            )
            st.altair_chart(chart, use_container_width=True)
...
def render_settings_page(api_client: APIClient):
    ...
    with col1:
        ...
        st.text_input("Email", value=user_data.get('email', ''), placeholder="No email set")
    ...
    with col1:
        default_max_results = st.slider("Default Max Results", 3, 20, 
                                       SessionManager.get_user_preference('max_results', 5))
        include_metadata_default = st.checkbox("Include metadata by default", 
    ...
    with col1:
        default_chunk_size = st.slider("Default Chunk Size", 500, 2000, 
                                     SessionManager.get_user_preference('chunk_size', 1000), step=100)
    with col2:
        default_chunk_overlap = st.slider("Default Chunk Overlap", 50, 500, 
                                        SessionManager.get_user_preference('chunk_overlap', 200), step=50)
