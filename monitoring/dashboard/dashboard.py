...
class DashboardAPI:
    ...
    def get_dashboard_data(self, days: int = 7) -> Optional[Dict]:
        ...
        try:
            response = self.session.get(f"{self.base_url}/metrics/dashboard?days={days}")
            if response.status_code == 200:
    ...
    def get_performance_metrics(self, days: int = 7) -> Optional[Dict]:
        ...
        try:
            response = self.session.get(f"{self.base_url}/metrics/performance?days={days}")
            if response.status_code == 200:
                return response.json()
            return None
        except Exception as e:
...
def create_monitoring_charts(performance_data: Dict) -> Dict:
    ...
    # System Load Gauge
    load_fig = go.Figure(go.Indicator(
    ...
    return {
        "response_time_dist": hist_fig,
        "system_health_gauge": load_fig
    }
...
def render_dashboard():
    ...
    if not st.session_state.authenticated:
        ...
        password = st.sidebar.text_input("Password", type="password", value=ADMIN_PASSWORD)
    ...
    st.markdown(f"**Time Range:** {selected_range} | **Last Updated:** {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    ...
    with health_cols[0]:
        ...
        st.markdown(f'<div class="status-indicator {status_class}"></div>**System Status:** {status.title()}', 
                   unsafe_allow_html=True)
    
    with health_cols[1]:
        db_status = health_data.get("database", {}).get("status", "unknown") if health_data else "error"
        db_class = "status-healthy" if db_status == "connected" else "status-error"
        st.markdown(f'<div class="status-indicator {db_class}"></div>**Database:** {db_status.title()}', 
                   unsafe_allow_html=True)
    
    with health_cols[2]:
        vector_status = health_data.get("vector_database", {}).get("status", "unknown") if health_data else "error"
        vector_class = "status-healthy" if vector_status == "healthy" else "status-error"
        st.markdown(f'<div class="status-indicator {vector_class}"></div>**Vector DB:** {vector_status.title()}', 
                   unsafe_allow_html=True)
    
    with health_cols[3]:
        openai_status = health_data.get("openai", "not_configured") if health_data else "error"
        openai_class = "status-healthy" if openai_status == "configured" else "status-warning"
        st.markdown(f'<div class="status-indicator {openai_class}"></div>**OpenAI:** {openai_status.title()}', 
                   unsafe_allow_html=True)
    ...
    chart_tabs = st.tabs(["📊 Overview", "📈 Performance", "🔍 Monitoring", "📋 Data Tables"])
    ...
    with chart_tabs[1]:
        col1, col2 = st.columns(2)
        with col1:
            st.plotly_chart(monitoring_charts["response_time_dist"], use_container_width=True)
        with col2:
            st.plotly_chart(monitoring_charts["system_health_gauge"], use_container_width=True)
    ...
    with chart_tabs[2]:
        ...
        activity_data = {
            ...
            "Event": ["Query processed", "Document uploaded", "User login", "Feedback received"] * 3,
        ...
        if dashboard_data["error_count"] > 0:
            st.warning(f"⚠️ {dashboard_data['error_count']} errors detected in the last {selected_range.lower()}")
        
        if performance_data["avg_response_time"] > 2.0:
            st.warning(f"⚠️ Response time ({performance_data['avg_response_time']:.2f}s) exceeds target (<2s)")
        
        if performance_data["success_rate"] < 0.95:
            st.error(f"🔴 Success rate ({performance_data['success_rate']*100:.1f}%) below target (95%)")
        
        if not any([dashboard_data["error_count"] > 0, performance_data["avg_response_time"] > 2.0, performance_data["success_rate"] < 0.95]):
    ...
    with chart_tabs[3]:
        ...
        feedback_df = pd.DataFrame({
            "Rating": [f"{k} Stars" for k in dashboard_data["feedback_distribution"].keys()],
    ...
    # Footer
    st.markdown("---")
    st.markdown(f"**Dashboard Version:** 1.0 | **API Endpoint:** {API_BASE_URL} | **Data Refresh:** {datetime.now().strftime('%H:%M:%S')}")
